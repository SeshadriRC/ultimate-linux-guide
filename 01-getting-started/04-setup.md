...
- Create a folder with name `ubuntu-data` in folder.

- Then run the below command in `poweshell` as run as administrator
...
docker run -dit `
  ...
  --cpus="2" `
  --memory="4g" `
  --mount type=bind,source="E:\ubuntu-data",target=/data `
